...
const pollButt = document.querySelector('.poll');
...
cutCall.addEventListener('click', () => {
    console.log('Cut Call button clicked')
    socket.emit('leave call');
    window.location.href = 'http://localhost:3000/feedback';
})

socket.on('remove peer', (sid) => {
    // Remove video element associated with the disconnected peer
    const videoElement = document.getElementById(`video-${sid}`);
    if (videoElement) {
        videoElement.remove();
        console.log(`Removed video element for peer: ${sid}`); // Debugging output
    }})
